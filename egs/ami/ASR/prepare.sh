...
#
dl_dir=$PWD/download
cuda_cmd="utils/queue-freegpu.pl --gpu 1 --config conf/gpu.conf --mem 8G"
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  for mic in ihm sdm mdm; do
    lhotse prepare ami --mic $mic --partition full-corpus-asr --normalize-text kaldi $dl_dir/amicorpus data/manifests/
  done
fi
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ] && [ $use_gss = true ]; then
  ...
  local/prepare_ami_gss.sh $dl_dir/amicorpus data/manifests exp/ami_gss
fi

if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  log "Stage 4: Compute fbank features for AMI"
  mkdir -p data/fbank
  $cuda_cmd exp/extract_feats_ami_test.log python local/compute_fbank_ami.py
  # log "Combine features from train splits"
  # pieces=$(find data/manifests -name "cuts_train_[0-9]*.jsonl.gz")
  # lhotse combine $pieces - | shuf | gzip -c > data/manifests/cuts_train.jsonl.gz
fi

if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  log "Stage 5: Compute fbank features for musan"
  mkdir -p data/fbank
  $cuda_cmd exp/extract_feats_musan.log python local/compute_fbank_musan.py
fi

if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  log "Stage 6: Dump transcripts for BPE model training. We also apply text normalization similar to Kaldi."
  mkdir -p data/lm
  cat <(gunzip -c data/manifests/ami-sdm_supervisions_train.jsonl.gz | jq '.text' |\
        perl -pe 's/[\.\,\?\!\:][0-9]+//g' |\
        perl -pe "s/[A-Z']+,[A-Z']+/ /g" |\
        perl -pe 's/\-/ /g' |\
        perl -pe 's/\_/. /g' |\
        ...
        perl -pe 's/^\-$//g' |\
        perl -pe 's/\s+\-$//' |\
        perl -pe 's/MM HMM/MM\-HMM/g' |\
        perl -pe 's/UH HUH/UH\-HUH/g')> data/lm/transcript_words.txt
fi
