...
class Zipformer2(EncoderInterface):
    ...
    def __init__(
        ...
        aux_output_layer: Optional[int] = None,
        ...
        self.aux_output_layer = aux_output_layer
    ...
    def forward(
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            ...
            if num_prefix_frames is None:
                attn_mask = self._get_attn_mask(x, chunk_size, left_context_chunks)
            else:
                attn_mask_with_prefix = self._get_attn_mask(
                    x, chunk_size, left_context_chunks
                )
                attn_mask_without_prefix = self._get_attn_mask(
                    x[num_prefix_frames:, ...], chunk_size, left_context_chunks
                )
        ...
        for i, module in enumerate(self.encoders):
            ...
            if num_prefix_frames is not None:
                attn_mask = (
                    attn_mask_with_prefix
                    if i <= self.aux_output_layer
                    else attn_mask_without_prefix
                )
            ...
            # Remove the prefixed part if we have reached the aux output layer.
            if i == self.aux_output_layer and num_prefix_frames is not None:
                x = x[num_prefix_frames:, ...]
                x_lens = x_lens - num_prefix_frames
                src_key_padding_mask = (
                    None
                    if src_key_padding_mask is None
                    else src_key_padding_mask[..., num_prefix_frames:]
                )
        ...
        if self.aux_output_layer is None:
            return x, lengths
        elif self.aux_output_layer == -1:
            return x, lengths, x_
        else:
            return x, lengths, outputs[self.aux_output_layer]
