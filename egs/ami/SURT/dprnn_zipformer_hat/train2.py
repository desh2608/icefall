...
def compute_heat_loss(x_masked, batch, num_channels=2) -> Tensor:
    ...
    for i in range(num_channels):
        target = torch.ones_like(x_masked[i]) * pad_value
    ...
    for b in range(B):
        ...
        for source_feat, (start, end) in zip(cut_source_feats, cut_source_boundaries):
            assigned = False
            source_feat = torch.exp(source_feat)
            for i in range(num_channels):
        ...
        if is_training and params.heat_loss_scale > 0.0:
            try:
                ...
                if "must match" in str(e):
                    logging.warning("Off-by-one error in tensor shapes. Skipping.")
...
def run(rank, world_size, args):
    ...
    train_cuts = ami.aimix_train_cuts(
        rvb_affix="comb",
        type_affix="v1",
        sources=True,
