...
#
#  - $dl_dir/amicorpus
#      Download using `lhotse download ami --mic mdm`
#
# We use pre-trained AMI models to decode the audio. Please
# download from the following link:
# https://huggingface.co/desh2608/icefall-asr-ami-pruned-transducer-stateless7
# - data/lang_bpe_500
...
if [ -z $rttm_affix ]; then
  dev_sdm_supervisions_file=ami-sdm_supervisions_dev.jsonl.gz
  test_sdm_supervisions_file=ami-sdm_supervisions_test.jsonl.gz
  dev_mdm_supervisions_file=ami-mdm_supervisions_dev.jsonl.gz
  test_mdm_supervisions_file=ami-mdm_supervisions_test.jsonl.gz
else
  rttm_dir=$dl_dir/$rttm_affix
  dev_sdm_supervisions_file=ami-sdm_supervisions_dev_${rttm_affix}.jsonl.gz
  test_sdm_supervisions_file=ami-sdm_supervisions_test_${rttm_affix}.jsonl.gz
  dev_mdm_supervisions_file=ami-mdm_supervisions_dev_${rttm_affix}.jsonl.gz
  test_mdm_supervisions_file=ami-mdm_supervisions_test_${rttm_affix}.jsonl.gz
fi
...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
  ...
  if [ ! -d $dl_dir/amicorpus ]; then
    lhotse download ami $dl_dir --mic mdm
  fi
fi
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ] && [ ! -z $rttm_affix ]; then
  ...
  gss utils rttm-to-supervisions --channels 1 $rttm_dir/dev data/manifests/$dev_sdm_supervisions_file
  gss utils rttm-to-supervisions --channels 1 $rttm_dir/test data/manifests/$test_sdm_supervisions_file
  gss utils rttm-to-supervisions --channels 8 $rttm_dir/dev data/manifests/$dev_mdm_supervisions_file
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  ...
  lhotse cut simple --force-eager \
    -r data/manifests/ami-mdm_recordings_dev.jsonl.gz \
    -s data/manifests/$dev_mdm_supervisions_file \
    $gss_exp_dir/dev/cuts.jsonl.gz

  lhotse cut simple --force-eager \
    -r data/manifests/ami-mdm_recordings_test.jsonl.gz \
...
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
  ...
  python local/compute_fbank_ami.py --mic ihm --data-dir data/manifests \
    --output-dir data/fbank --dataset-parts dev test
fi

if [ $stage -le 8 ] && [ $stop_stage -ge 8 ]; then
  log "Stage 8: Compute fbank features for AMI SDM data"
  python local/compute_fbank_ami.py --mic sdm --data-dir data/manifests \
    --output-dir data/fbank --rttm-affix "$rttm_affix" --dataset-parts dev test
fi
