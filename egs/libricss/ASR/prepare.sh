...
if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
    ...
    $cmd JOB=1:$nj_gss $gss_exp_dir/log/enhance.JOB.log \
        gss enhance cuts \
          ...
          --num-buckets 3 \
          --enhanced-manifest $gss_exp_dir/cuts${gss_affix}.JOB.jsonl.gz \
          --force-overwrite

    # Merge the enhanced cuts
    lhotse combine $gss_exp_dir/cuts${gss_affix}.*.jsonl.gz data/manifests/libricss-gss_cuts_all${rttm_affix}${gss_affix}.jsonl.gz
fi
...
if [ $stage -le 9 ] && [ $stop_stage -ge 9 ]; then
    ...
    python local/compute_fbank_libricss.py --mic gss --data-dir data/manifests \
