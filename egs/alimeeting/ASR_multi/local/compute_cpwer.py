...
def compute_cpWER(ref_text, hyp_text):
    ...
    for i in range(M):
        for j in range(N):
            cur_ref = [x for x in ref_text[i]]
            cur_hyp = [x for x in hyp_text[j]]
    ...
    for ref, hyp in zip(ref_text, hyp_text):
        ref = [x for x in ref.strip()]
        hyp = [x for x in hyp.strip()]
        count += len(ref)
...
def main(ref, hyp, stats_file):
    ...
    with stats_file.open("w") as f:
        json.dump(wer_dict, f, indent=2, ensure_ascii=False)
