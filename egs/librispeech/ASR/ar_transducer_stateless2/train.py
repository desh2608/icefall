...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "How many left context can be seen in chunks when calculating"
            " attention."
        ),
    )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=0.003,
        help=(
            "The initial learning rate.  This value should not need to "
            "be changed."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.25,
        help=(
            "The scale to smooth the loss with lm "
...
def compute_loss(
    ...
    """
    Compute alignment-restricted RNN-T loss given the model and its inputs.
    ...
    # The subsampling function below is based on the Conformer model with
    # 4x subsampling.
    ranges = get_prune_ranges(
        batch["supervisions"],
        x_lens=feature_lens,
        y_lens=y_lens,
        subsampling_function=lambda l: (((l - 1) >> 1) - 1) >> 1,
    ...
    with torch.set_grad_enabled(is_training):
        ...
        if not torch.all(pruned_loss_is_finite):
            logging.info(
                f"Not all losses are finite!\npruned_loss: {pruned_loss}"
            )
            ...
            if torch.all(~pruned_loss_is_finite):
                raise ValueError(
    ...
    assert pruned_loss.requires_grad == is_training
...
def run(rank, world_size, args):
    ...
    train_dl = librispeech.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict, bpe_model=sp
    )

    valid_cuts = librispeech.dev_clean_cuts()
    valid_cuts += librispeech.dev_other_cuts()
    valid_dl = librispeech.valid_dataloaders(valid_cuts, bpe_model=sp)
